...
class StableNormal(PluginCommandLineNode):

    commandLine = 'python ' + os.path.join(os.path.dirname(__file__), 'run.py') + ' {allParams}'
    ...
    category = 'MRRS - Normal Maps'
    documentation = '''TODO'''

    envType = EnvType.CONDA_CUSTOM
    envFile = os.path.join(os.path.dirname(__file__), "conda_env.yaml")
    ...
    outputs = [
        ...
        desc.File(
            ...
            semantic="image",
            value=desc.Node.internalFolder + "<VIEW_ID>_normalMap.png",
    ...
    @property
    def _envName(cls):
        return os.path.join(os.path.dirname(__file__), "conda_env")
